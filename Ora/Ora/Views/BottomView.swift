...
enum SelectedDateStep {
    ...
    var titleSubtitle: (title: String, subtitle: String) {
        ...
        case .comeBack:
            return ("Entry Added", "Come back tomorrow to track your period and flow again.")
        case .completed:
            return ("Entry Added", "Your period has ended.")
        }
    }
}
...
struct BottomView: View {
    ...
    private var SelectDateContinueView: some View {
        VStack(alignment: .center) {
            ...
            Text("Please select a date to track").font(.system(size: 25))
        ...
        private func FlowTypeButton(type: FlowType) -> some View {
            Button(action: {
                ...
                withAnimation {
                    ...
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.7) {
                        if let date = manager.currentSelectedDate {
                            let model = DateModel(date: date, dateType: .period, timeline: selectedDateTimeline, flowType: type)
...
struct BottomView_Previews: PreviewProvider {
    static var previews: some View {
        BottomView(manager: PeriodManager())
    }
}
